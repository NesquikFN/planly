...
function CalendarMain() {
  const { viewMode, goToPrevious, goToNext, syncError } = useCalendarStore();
  ...
  return (
    <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-white/8 dark:bg-surface lg:h-full">
      ...
      {/* Every failed write is already rolled back locally by the store, so
          what's on screen is accurate — this only explains why the change
          the user just made disappeared again. */}
      {syncError && (
        <p className="border-b border-red-100 bg-red-50 px-4 py-2 text-xs font-medium text-red-600 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-400">
          {syncError}
        </p>
      )}
