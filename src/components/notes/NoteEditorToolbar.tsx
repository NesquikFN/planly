...
function ActionIconButton({ icon: Icon, label, onClick }: { icon: LucideIcon; label: string; onClick: () => void }) {
  return (
    <button
      ...
      title={label}
      className={cn(
        "rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300",
        focusRing,
      )}
    >
      <Icon size={15} />
...
export function NoteEditorToolbar({
  ...
  return (
    <div className="sticky top-0 z-10 flex flex-wrap items-center gap-1 border-y border-gray-100 bg-white/95 px-4 py-2 backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/95">
      <DropdownMenu
        trigger={<span className="text-xs font-semibold text-gray-500 dark:text-gray-400">Aa</span>}
        triggerClassName={cn("rounded-lg px-2 py-1.5 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800", focusRing)}
        align="left"
      ...
      <DropdownMenu
        ...
        triggerClassName={cn("rounded-lg p-1.5 hover:bg-gray-50 dark:hover:bg-gray-800", focusRing)}
