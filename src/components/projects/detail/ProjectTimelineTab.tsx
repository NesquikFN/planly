...
export function ProjectTimelineTab({ project }: { project: Project }) {
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between">
      <h2 className="text-base font-semibold text-gray-900 dark:text-gray-50">Timeline проекта</h2>
      {canEdit(project) && (
        <button
          type="button"
    ...
    <section className={projectCard}>
      ...
      {items.length ? <ul className="mt-3 divide-y divide-gray-100 dark:divide-gray-800">{items.map(i => <li key={`${i.type}-${i.id}`} className="flex justify-between py-2 text-sm"><span>{i.title} <span className="text-xs text-gray-400">· {i.type}</span></span><time className="text-gray-400">{i.date}</time></li>)}</ul> : <p className="mt-3 text-sm text-gray-400 dark:text-gray-500">Нет задач и событий в выбранном периоде</p>}
    </section>

    <section className={projectCard}>
      <h3 className={projectSectionTitle}>Этапы проекта</h3>
      {project.milestones.length ? <ul className="mt-3 divide-y divide-gray-100 dark:divide-gray-800">{project.milestones.map(m => <li key={m.id} className="flex items-center justify-between py-2 text-sm"><span className={m.status === "upcoming" ? "text-gray-400 dark:text-gray-500" : "text-gray-700 dark:text-gray-200"}>{m.label}</span><span className="text-xs text-gray-400">{m.dateLabel}</span></li>)}</ul> : <p className="mt-3 text-sm text-gray-400 dark:text-gray-500">Этапы не заданы</p>}
    </section>
  </div>;
}
