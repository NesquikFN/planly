...
import { CalendarCheck, Clock3, TrendingUp } from "lucide-react";
  ...
  return (
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="mt-3 flex items-baseline gap-2">
        <span className="text-3xl font-semibold text-gray-900 dark:text-gray-50">{data.onTimePercent}%</span>
        <span className="text-sm text-gray-400 dark:text-gray-500">задач выполнено вовремя</span>
      </div>
      ...
      <div className="mt-3 flex items-center gap-4 text-xs">
        ...
        <span className="inline-flex items-center gap-1.5 text-gray-400 dark:text-gray-500">
          ...
          Прошлый период — {data.previousPeriodPercent}%
        </span>
        <span className="inline-flex items-center gap-1 font-medium text-emerald-600 dark:text-emerald-400">
          <TrendingUp size={13} />+{data.onTimePercent - data.previousPeriodPercent}%
        </span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2.5 border-t border-gray-100 pt-4 text-sm dark:border-gray-800">
        ...
        <div className="flex items-center gap-2.5 text-gray-500 dark:text-gray-400">
          ...
          <span className="font-medium text-gray-900 dark:text-gray-50">{data.mostDisciplinedDay}</span>
        </div>
        <div className="flex items-center gap-2.5 text-gray-500 dark:text-gray-400">
          <Clock3 size={15} className="shrink-0 text-amber-500" />
          Больше всего просрочек — <span className="font-medium text-gray-900 dark:text-gray-50">{data.mostOverdueDay}</span>
        </div>
      </div>
    </section>
  );
