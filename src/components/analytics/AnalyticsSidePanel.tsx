  ...
  return (
    <aside className="flex flex-col gap-4 lg:sticky lg:top-6 lg:self-start">
      <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500">задач с сегодняшней датой выполнено</p>
        <div className="mt-3 space-y-1.5 text-xs text-gray-500 dark:text-gray-400">
          <div className="flex items-center justify-between">
            <span>События сегодня</span>
            <span className="font-medium text-gray-900 dark:text-gray-50">{data.today.eventsLabel}</span>
          </div>
          <div className="flex items-center justify-between">
      ...
      {data.bestDay && (
        <section className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-50">Лучший день периода</h3>
          </div>
          <p className="mt-2 text-lg font-semibold text-gray-900 dark:text-gray-50">{data.bestDay.label}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500">Выполнено {data.bestDay.tasksDone} задач</p>
        </section>
      )}
