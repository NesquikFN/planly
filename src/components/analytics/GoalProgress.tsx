  ...
  return (
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-white/8 dark:bg-surface dark:shadow-none">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gray-900 dark:text-ink">Прогресс целей</h2>
        {goals.length > 0 && (
      ...
      {goals.length === 0 ? (
        <div className="mt-4 flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 py-8 text-center dark:border-white/8">
          <Target size={22} className="text-gray-300 dark:text-ink-faint" />
          <p className="mt-2 text-sm font-medium text-gray-500 dark:text-ink-faint">Цели пока не отслеживаются</p>
          ...
          {goals.map((goal) => (
            <div key={goal.id}>
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium text-gray-900 dark:text-ink">{goal.title}</span>
                <span className="shrink-0 text-sm font-semibold text-gray-900 dark:text-ink">{goal.percent}%</span>
              </div>
              <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-surface-2">
                <div
                  className={cn("h-full rounded-full", goal.onTrack ? "bg-accent" : "bg-amber-400")}
                  style={{ width: `${goal.percent}%` }}
                />
              </div>
              <div className="mt-1.5 flex items-center justify-between text-xs text-gray-400 dark:text-ink-faint">
