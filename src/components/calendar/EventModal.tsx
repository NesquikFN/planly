...
export function EventModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8">
      ...
      <div className="relative max-h-full w-full max-w-md overflow-y-auto rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Календарь</span>
            ...
            <span className="mt-1 block text-xs text-gray-400 dark:text-gray-500">Цвет события берётся из выбранного календаря.</span>
          </label>
          ...
          <label className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-600 dark:text-gray-300">Важное событие</span>
          </label>

          <label className="block">
            <span className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Описание</span>
            <textarea
              value={description}
              onChange={(event) => setDescription(event.target.value)}
              rows={2}
              className="w-full resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
            />
          </label>

          <div className="flex gap-3">
            <label className="block flex-1">
              <span className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Проект (необязательно)</span>
              <input
                type="text"
                value={project}
            ...
            <label className="block flex-1">
              <span className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Задача (необязательно)</span>
              <input
                type="text"
                value={task}
