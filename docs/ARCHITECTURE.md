...
> **Частично устарело.** Документ описывает фронтенд времён, когда данные жили только в
> `localStorage`, а репозиторий не был монорепо. Что изменилось:
>
> - все пути `src/...` ниже теперь читаются как `frontend/src/...`;
> - появился `backend/` — свой сервис на Express + Postgres, см. [BACKEND.md](BACKEND.md);
> - профиль, задачи и события календаря живут в облаке (`localStorage` для них — только кэш);
> - Supabase удалён целиком, прежние миграции лежат в `database/legacy-supabase/`;
> - `lib/supabase/*` заменён на `lib/api-client.ts`.
>
> Разделы про модули без стора (Projects, Notes, Reminders, Analytics) по-прежнему верны.
